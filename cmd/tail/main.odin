...
USAGE :: `Usage: tail [-n COUNT] [--help] [--version] file ...
...
main :: proc() {
	...
	for i < len(raw_args) {
		...
		if arg == "-n" || arg == "--lines" {
			i += 1
			if i >= len(raw_args) {
				winconsole.write_string(errw, "tail: option requires an argument -- 'n'\r\nTry 'tail --help'.\r\n")
				os.exit(2)
			}
			n, ok := strconv.parse_int(raw_args[i], 10)
			if !ok || n < 0 {
				winconsole.write_string(errw, "tail: invalid number of lines: '")
	...
	for path, idx in parsed.rest {
		...
		err := write_tail(path, line_count)
