...
physical_returns_absolute_backslashed_path :: proc(t: ^testing.T) {
    ...
    p, err := get_cwd_physical(context.allocator)
    defer delete(p)
...
logical_with_unset_pwd_matches_physical :: proc(t: ^testing.T) {
    ...
    defer sync.unlock(&cwd_mu)
    os.unset_env("PWD")
    p_log, err1 := get_cwd_logical(context.allocator)
    defer delete(p_log)
    p_phy, err2 := get_cwd_physical(context.allocator)
    defer delete(p_phy)
...
logical_with_lowercase_pwd_returns_pwd_value :: proc(t: ^testing.T) {
    sync.lock(&cwd_mu)
    defer sync.unlock(&cwd_mu)
    p_phy, err1 := get_cwd_physical(context.allocator)
    defer delete(p_phy)
    testing.expect_value(t, err1, Error.None)
    ...
    p_log, err2 := get_cwd_logical(context.allocator)
    defer delete(p_log)
    testing.expect_value(t, err2, Error.None)
    testing.expect_value(t, p_log, p_phy)
}
