...
@(test)
prints_first_n_lines :: proc(t: ^testing.T) {
	base := temp_base()
	defer delete(base)
	dir := strings.concatenate({base, "\\winix-head-basic"}, context.allocator)
	defer delete(dir)
...
default_is_ten_lines :: proc(t: ^testing.T) {
	base := temp_base()
	defer delete(base)
	dir := strings.concatenate({base, "\\winix-head-default"}, context.allocator)
	defer delete(dir)
	os.make_directory(dir)
	defer os.remove_all(dir)
...
short_file_prints_all :: proc(t: ^testing.T) {
	base := temp_base()
	defer delete(base)
	dir := strings.concatenate({base, "\\winix-head-short"}, context.allocator)
	defer delete(dir)
	os.make_directory(dir)
	defer os.remove_all(dir)
...
missing_file_exits_1 :: proc(t: ^testing.T) {
	...
	_, errb, code := run(t, []string{"nonexistent_winix_head.txt"}, base)
	defer delete(errb)
